...
(test-group
  ...
  (begin ; tokens starting with #
    ...
    ; case insensitive, error must be caught by the parser.
    ...
    ; invalid characters are parsed as well
    ; parser must generate errors for them
    (test-complete-tokens 'char "#\\dElEte")
    (test-complete-tokens 'char "#\\Space")
